...
import static cn.liyu.hospital.dto.TimePeriodEnum.AM;
import static cn.liyu.hospital.dto.TimePeriodEnum.PM;
...
public class VisitAppointmentServiceImpl implements IVisitAppointmentService {
     ...
     *
     * @param param 就诊预约参数
     * @return 新生成的预约编号（失败返回 null）
     */
    @Override
    public Long insert(VisitAppointmentParam param) {
        ...
        int rows = appointmentMapper.insertSelective(appointment);
        return rows > 0 ? appointment.getId() : null;
    }

    /**
     * 更新预约状态
     * <p>
     * 状态机校验：仅"待就诊(0)"可流转到 失约(1)/取消(2)/完成(3)，
     * 防止已取消的单被标记完成、已完成的单被重复操作或误触发退款
     *
     * @param id     预约编号
    ...
    public boolean update(Long id, Integer status) {
        // 状态机校验：当前必须为"待就诊"
        VisitAppointment current = appointmentMapper.selectByPrimaryKey(id);
        if (current == null || !WAITING.getStatus().equals(current.getStatus())) {
            return false;
        }
     ...
     */
    @Override
    public List<VisitAppointment> listByAccountId(Long accountId, Integer status, Integer pageNum, Integer pageSize) {
    ...
    public int getQueueNum(VisitAppointment appointment) {
        ...
        // 当前时间段对应的排队号
        Integer tp = appointment.getTimePeriod();
        if (tp == null) tp = 1;
        return MAX_PEOPLE_OF_TIME_PERIOD * (tp - 1) + peopleBeforeTimePeriod;
    }
    ...
    public List<VisitUserInfoDTO> listVisitUserInfo(Long doctorId, Integer time, Date day, Integer pageNum, Integer pageSize) {
        ...
        // 如果传了医生/时间筛选，先查计划再查预约（startPage 必须在 Plan 查询之后）
        if (doctorId != null && time != null && day != null) {
        ...
        // 无筛选条件：查全部预约（分页）
        PageHelper.startPage(pageNum, pageSize);
        VisitAppointmentExample allExample = new VisitAppointmentExample();
        allExample.setOrderByClause("gmt_create desc");
        allExample.createCriteria().andStatusNotEqualTo(CANCEL.getStatus());
        return appointmentMapper.selectByExample(allExample).stream()
                .map(this::convertToUserInfo)
                .collect(Collectors.toList());
    }

    /**
     * 管理端预约列表（聚合病人信息 + 出诊计划信息）
     * <p>
     * 与用户端 search 接口的区别：不按登录人 accountId 过滤，管理员可查看全部病人的预约
     *
     * @param cardId   就诊卡编号（可空）
     * @param doctorId 医生编号（可空）
     * @param day      出诊日期（可空）
     * @param status   预约状态（可空）
     * @param pageNum  第几页
     * @param pageSize 页大小
     * @return 管理端预约列表
     */
    @Override
    public List<AdminAppointmentDTO> listAdminAppointments(Long cardId, Long doctorId, Date day, Integer status,
                                                           Integer pageNum, Integer pageSize) {

        // 按医生/日期筛选时，先查出符合条件的出诊计划编号（startPage 必须在 Plan 查询之后）
        List<Long> planIds = null;
        if (doctorId != null || day != null) {
            planIds = visitPlanService.listPlanIds(doctorId, day);
    ...
    /**
     * 转换为管理端预约信息（聚合患者姓名 + 出诊计划的医生/医院/日期）
     *
     * @param appointment 预约记录
     * @return 管理端预约信息
     */
    private AdminAppointmentDTO convertToAdmin(VisitAppointment appointment) {

        AdminAppointmentDTO dto = new AdminAppointmentDTO();

        dto.setAppointmentId(appointment.getId());
        dto.setPlanId(appointment.getPlanId());
        dto.setCardId(appointment.getCardId());
        dto.setTimePeriod(appointment.getTimePeriod());
        dto.setStatus(appointment.getStatus());
        dto.setPayStatus(appointment.getPayStatus());
        dto.setGmtCreate(appointment.getGmtCreate());

        // 患者姓名（就诊卡）
        dto.setPatientName(userMedicalCardService.getName(appointment.getCardId()));

        // 出诊计划信息（医生/医院/日期，详情查询走缓存）
        visitPlanService.getOptional(appointment.getPlanId()).ifPresent(plan -> {
            dto.setDoctorName(plan.getDoctorName());
            dto.setHospitalName(plan.getHospitalName());
            dto.setDay(plan.getDay());
    ...
    @Override
    public VisitAppointmentDTO convertToDTO(VisitAppointment appointment) {
    ...
    /**
     * Stream 消费者专用下单（带事务，供 AppointmentStreamConsumer 调用）
     */
    @Override
    @org.springframework.transaction.annotation.Transactional(rollbackFor = Exception.class)
    public boolean insertStreamOrder(VisitAppointment appointment) {
